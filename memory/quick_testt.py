...
def test_full_system() -> bool:
    ...
    # Without a scratchpad goal, these messages score below the promotion
    # threshold (base 0.3 + role 0.1 + novel 0.1 = 0.5 < 0.6) and never reach
    # episodic/semantic memory - so search would return 0 matches regardless
    # of whether search itself works. Setting a goal (+0.15) is what actually
    # gets them promoted, so this test exercises the real end-to-end path.
    mm.update_scratchpad(plan='Determine whether cats can fly')

    mm.add_message('user', 'I believe cats are able to fly')
    mm.add_message('user', 'Actually, cats are not able to fly')
    ...
    print(f"Consolidation results: {results.get('contradictions_found', 0)} contradictions found, "
          f"{results.get('facts_created', 0)} facts created")
    ...
    mm.end_session()
    return len(search_results['semantic']) > 0
